...
if __name__ == "__main__":
    # Configuration
    config = ETTConfig(
        base_path=r"C:\Users\ogbonda\DOCUMENTS\gridx-datasets\etdataset\ETT-small"
    )
    
    # Initialize processor
    processor = ETTDataProcessor(config)
    
    # Load datasets
    stats = processor.load_ett_datasets()
    print("Loading Statistics:")
    for key, value in stats.items():
        print(f"  {key}: {value}")
        
    # Create samples
    samples_dict = processor.create_ett_samples(anomaly_detection=True)
    
    # Process one dataset for demonstration
    if samples_dict:
        dataset_name = list(samples_dict.keys())[0]
        samples = samples_dict[dataset_name][:1000]  # First 1000 samples
        
        print(f"\nProcessing {dataset_name} ({len(samples)} samples):")
        
        # Extract features
        features_df = processor.extract_operational_features(samples)
        print(f"  Features extracted: {features_df.shape}")
        
        # Create maintenance targets
        targets_df = processor.create_maintenance_targets(samples)
        print(f"  Maintenance targets: {targets_df.shape}")
        
        # Show summary
        summary = processor.get_dataset_summary()
        print(f"\nDataset Summary:")
        print(f"  Total samples: {summary['total_samples']:,}")
        print(f"  Date range: {summary['overall_date_range']['start']} to {summary['overall_date_range']['end']}")
